...
class Leduc(object):
    ...
    card_num: dict[InfoSetPtr, dict[act_t, int]] = {}
...
def create_tree():
    ...
    act_map: dict[player_t, dict[InfoSetPtr, list[act_t]]] = {LUCK: {head_I: act_t.cast(["J", "Q", "K"])}, A: {}, B: {}}

    head_I.o.append(true_root)
    true_root.o.I = head_I
    card_num = Leduc.card_num
    card_num[head_I] = {act_t("J"): 2, act_t("Q"): 2, act_t("K"): 2}
    ...
    # [print(item, card_num[item]) for item in card_num]
...
if __name__ == "__main__":
    ...
    for obs in info_collect[LUCK]:
        ...
        card_num_sum = 0
        for a in Leduc.card_num[I]:
            card_num_sum += Leduc.card_num[I][a]
        for i, a in enumerate(act_map[LUCK][I]):
            sigma[LUCK][I][i] = Leduc.card_num[I][a] / card_num_sum
    for player_i in [A, B]:
