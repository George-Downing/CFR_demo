...
if __name__ == "__main__":
    ...
    print("build tree" + " -" * 50)
    Node.roots.append(Node(NodePtr(0), act_t("CCCCCCCC")).p)
    root = Node.roots[-1]
    for a in act_t.cast(["r", "s", "p"]):
        root.o.child[a] = Node(root, a).p
    print("Node.roots", Node.roots)
    ...
    print("flush memory" + " -" * 50)
    with open("Node.roots.pkl", "wb") as f:
        pickle.dump(Node.roots, f)
    with open("Node.HEAP.pkl", "wb") as f:
    ...
    print("load old" + " -" * 50)
    with open("Node.roots.pkl", "rb") as f:
        Node_roots_old: list[NodePtr] = pickle.load(f)
    with open("Node.HEAP.pkl", "rb") as f:
        Node_HEAP_old: dict[NodePtr, Node] = pickle.load(f)
    with open("money.pkl", "rb") as f:
        money_old: dict[NodePtr, float] = pickle.load(f)
    root_old = Node_roots_old[-1]
    print("Node_roots_old", Node_roots_old)
    # print("Node_HEAP_old[root_old].child", Node_HEAP_old[root_old].child)
    print("money_old", money_old)

    print("build tree" + " -" * 50)
    Node.roots.append(Node(NodePtr(0), act_t("CCCCCCCC")).p)
    root: NodePtr = Node.roots[-1]
    for a in act_t.cast(["r", "s", "p"]):
        root.o.child[a] = Node(root, a).p
    print("Node.roots", Node.roots)
    print("Node.HEAP", Node.HEAP)

    print("ptr_matching" + " -" * 50)
    ptr_forward = {root_old: root}
    ptr_reverse = {root: root_old}
    for a in Node_HEAP_old[root_old].child:
        ch_old = Node_HEAP_old[root_old].child[a]
        ch = root.o.child[a]
        ptr_forward[ch_old] = ch
        ptr_reverse[ch] = ch_old
    print("ptr_forward", ptr_forward)
    print("ptr_reverse", ptr_reverse)

    print("money" + " -" * 50)
    for n in money_old:
        money[ptr_forward[n]] = money_old[n]
    print("money", money)
